...
<body>
    ...
    <div style="margin-left: 3rem; margin-top: 1rem;" class="details font-family">
        ...
        <div class="information messages font-family" style="margin-top: 1rem;">
            <div><span style="color: #602b70;font-weight: bold;">Mitteilungen:</span></div><br>
            <div>
            @if($conversation != null)
                @foreach($conversation['chats'] as $chat)
                    @if($chat->sender_id == $user->id)
                    <div style="background-color: #d8bce1; border-radius: 30px;margin-bottom: 5px; padding: 0.3rem; padding-left: 1rem;">{{$chat->created_at}} ({{$user->first_name}}) :{{$chat->message}}</div>
                    @else
                    <div style="background-color: #AC79BC; border-radius: 30px;margin-bottom: 5px; padding: 0.3rem; padding-left: 1rem;">{{$chat->created_at}} ({{$provider->first_name}}) :{{$chat->message}}</div>
                    @endif
                @endforeach
            @endif
            </div>
