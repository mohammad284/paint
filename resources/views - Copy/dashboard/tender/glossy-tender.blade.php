...
<div class="row">
    <div class="col-md-12 grid-margin stretch-card">
        <div class="card">
        <div class="card-body">
            <h6 class="card-title">{{__('مناقصات اللكر')}}</h6>
                    ...
                    <tbody>
                        @foreach($tenders as $tender)
                            <tr>
                                ...
                                <td>{{$tender->note}}</td>
                                <td>{{$tender->text}}</td>
                                <td>{{count($tender['glossy'])}}</td>
                                <td>{{$tender['postal']->Postal_Code}}</td>
                                <td>{{$tender['postal']->Admin_Name}}</td>
                                <td>{{$tender->expected_date}}</td>
                                <td class="action"> 
                                    <a  title="{{__('مزيد من التفاصيل')}}" href="/admin/glossyDetails/{{$tender->id}}"> <i data-feather="eye"></i></i></a>
